<div class="max-w-4xl mx-auto">
    <div class="mb-8">
        <p class="text-sm font-mono text-cyan-400 mb-1"><span class="text-gray-500">//</span> volunteering</p>
        <h1 class="text-2xl font-bold gradient-text">Manage Volunteering</h1>
    </div>
    ...
    <div class="glass-card rounded-xl p-6 mb-8">
        ...
        <form method="POST" action="/admin/volunteering" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <input type="text" name="place" maxlength="100" placeholder="e.g. Qatar"
                       class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
            </div>
            <div>
            ...
            <div>
                ...
                <input type="text" name="start_date" maxlength="50" placeholder="e.g. 2022"
                       class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
            </div>
            <div>
                <label class="block text-xs text-gray-400 mb-1">End Date</label>
                <input type="text" name="end_date" maxlength="50" placeholder="e.g. Present"
                       class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
            </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-blue-500/10 text-gray-400 text-left">
                    ...
                    <th class="p-4 font-medium hidden sm:table-cell">Period</th>
                    <th class="p-4 font-medium">Actions</th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($entries)): ?>
                <tr><td colspan="4" class="p-8 text-center text-gray-500">No volunteer entries yet.</td></tr>
                <?php else: ?>
                <?php foreach ($entries as $e): ?>
                <tr class="border-b border-blue-500/5 hover:bg-blue-500/5">
                    <td class="p-4 text-white"><?= htmlspecialchars($e['title']) ?></td>
                    <td class="p-4 hidden sm:table-cell text-gray-400"><?= htmlspecialchars($e['organization']) ?></td>
                    <td class="p-4 hidden sm:table-cell text-gray-500">
                        ...
                        <div id="edit-<?= (int) $e['id'] ?>" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/60" @click.self="document.getElementById('edit-<?= (int) $e['id'] ?>').classList.add('hidden')">
                            <div class="glass rounded-xl p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
                                ...
                                <form method="POST" action="/admin/volunteering/<?= (int) $e['id'] ?>" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        <label class="block text-xs text-gray-400 mb-1">Place</label>
                                        <input type="text" name="place" value="<?= htmlspecialchars($e['place'] ?? '') ?>" maxlength="100"
                                               class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-400 mb-1">Link</label>
                                        <input type="url" name="link" value="<?= htmlspecialchars($e['link'] ?? '') ?>" maxlength="255"
                                               class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-400 mb-1">Start Date</label>
                                        <input type="text" name="start_date" value="<?= htmlspecialchars($e['start_date'] ?? '') ?>" maxlength="50"
                                               class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-gray-400 mb-1">End Date</label>
                                        <input type="text" name="end_date" value="<?= htmlspecialchars($e['end_date'] ?? '') ?>" maxlength="50"
                                               class="w-full px-3 py-2 rounded-lg bg-gray-800/50 border border-gray-700 text-white text-sm focus:border-blue-500">
                                    </div>
                                    <div class="sm:col-span-2">
                                        <label class="block text-xs text-gray-400 mb-1">Description</label>

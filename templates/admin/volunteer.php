<div class="term-section">
    <p class="term-prompt text-cat-green text-xs font-mono mb-1"><?= \App\Helpers\Language::t('admin.volunteering.cmd') ?></p>
    <h1 class="text-lg font-bold text-cat-mauve font-mono mb-6"><?= \App\Helpers\Language::t('admin.volunteering.title') ?></h1>
    ...
    <div class="term-panel p-4 mb-6">
        <h2 class="text-sm font-bold text-cat-peach font-mono mb-4"><?= \App\Helpers\Language::t('admin.volunteering.add') ?></h2>
        <form method="POST" action="/admin/volunteering" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                <label class="block text-xs text-cat-overlay2 font-mono mb-1"><?= \App\Helpers\Language::t('admin.volunteering.start_date') ?></label>
                <input type="text" name="start_date" maxlength="50" placeholder="<?= \App\Helpers\Language::t('admin.volunteering.start_date_placeholder') ?>" class="w-full px-3 py-2 text-sm">
            </div>
            <div>
                <label class="block text-xs text-cat-overlay2 font-mono mb-1"><?= \App\Helpers\Language::t('admin.volunteering.end_date') ?></label>
                <input type="text" name="end_date" maxlength="50" placeholder="<?= \App\Helpers\Language::t('admin.volunteering.end_date_placeholder') ?>" class="w-full px-3 py-2 text-sm">
            </div>
            ...
            <div class="flex items-end">
                <button type="submit" class="term-btn term-btn-primary text-sm w-full"><?= \App\Helpers\Language::t('admin.volunteering.add_btn') ?></button>
            </div>
        </form>
    </div>
        ...
        <table class="w-full text-sm">
            <thead>
                <tr class="border-b border-cat-surface1 text-cat-overlay2 text-left">
                    <th class="p-4 font-mono font-medium"><?= \App\Helpers\Language::t('admin.timeline.title_field') ?></th>
                    <th class="p-4 font-mono font-medium hidden sm:table-cell"><?= \App\Helpers\Language::t('admin.timeline.organization') ?></th>
                    <th class="p-4 font-mono font-medium hidden sm:table-cell"><?= \App\Helpers\Language::t('admin.timeline.period') ?></th>
                    <th class="p-4 font-mono font-medium"><?= \App\Helpers\Language::t('admin.languages.actions') ?></th>
                </tr>
            </thead>
            <tbody>
                <?php if (empty($entries)): ?>
                <tr><td colspan="4" class="p-8 text-center text-cat-overlay0 font-mono text-xs"><?= \App\Helpers\Language::t('admin.volunteering.empty') ?></td></tr>
                <?php else: ?>
                ...
                <tr class="border-b border-cat-surface0 hover:bg-cat-surface0/50">
                    ...
                    <td class="p-4">
                        <button @click="document.getElementById('edit-<?= (int) $e['id'] ?>').classList.toggle('hidden')"
                                class="text-xs px-2 py-1 rounded bg-cat-surface0 text-cat-blue hover:bg-cat-surface1 font-mono"><?= \App\Helpers\Language::t('admin.volunteering.edit') ?></button>
                        <form method="POST" action="/admin/volunteering/<?= (int) $e['id'] ?>" class="inline" onsubmit="return confirm('<?= \App\Helpers\Language::t('admin.volunteering.delete_confirm') ?>')">
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" name="csrf_token" value="<?= \App\Core\Session::getCsrfToken() ?>">
                            <button type="submit" class="text-xs px-2 py-1 rounded bg-cat-surface0 text-cat-red hover:bg-cat-surface1 font-mono"><?= \App\Helpers\Language::t('admin.volunteering.delete') ?></button>
                        </form>
                        ...
                        <div id="edit-<?= (int) $e['id'] ?>" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-cat-base/80" @click.self="document.getElementById('edit-<?= (int) $e['id'] ?>').classList.add('hidden')">
                            <div class="term-panel p-6 w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
                                <h3 class="text-sm font-bold text-cat-peach font-mono mb-4"><?= \App\Helpers\Language::t('admin.volunteering.edit_title') ?></h3>
                                <form method="POST" action="/admin/volunteering/<?= (int) $e['id'] ?>" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    ...
                                    <div class="flex items-end gap-2">
                                        <button type="submit" class="term-btn term-btn-primary text-sm flex-1"><?= \App\Helpers\Language::t('admin.volunteering.save') ?></button>
                                        <button type="button" @click="document.getElementById('edit-<?= (int) $e['id'] ?>').classList.add('hidden')"
                                                class="term-btn text-sm flex-1"><?= \App\Helpers\Language::t('admin.volunteering.cancel') ?></button>
                                    </div>
                                </form>
                            </div>
                        </div>
